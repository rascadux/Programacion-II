...
if [ "$MAIN_NAME" = "main" ]
then
  ficherosEntrada=("script_minimos/new" "script_minimos/bid" "script_minimos/delete" "script_minimos/award" "script_minimos/withdraw" "script_minimos/remove")
  ficherosRef=("script_minimos/new_ref" "script_minimos/bid_ref" "script_minimos/delete_ref" "script_minimos/award_ref" "script_minimos/withdraw_ref" "script_minimos/remove_ref")
else
  ficherosEntrada=("")
...
then
  ...
  show_result "Checkpoint #2 result (April 22th):" ${CHECKPOINT_OK}
...
#8 de abril, primer seguimiento
  ...
  show_result "Bid stack test:" ${STACK_OK}
  show_result "Product list test:" ${LIST_OK}
  show_result "Tests global result (checkpoint #1 - April 8th):" $((${STACK_OK} * ${LIST_OK}))
